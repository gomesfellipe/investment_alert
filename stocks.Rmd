---
title: "Como desenvolver um robô que envia resultados de Machine Learning para seu Smartphone e ajuda nos investimentos"
subtitle: "Entenda a lógica de como montar uma carteira, coletar dados de finança em tempo real, treinar um modelo de Machine Learning e receber análises automatizadas no Smartphone"
...
Já estamos no início de 2020 e desde 2019 já liam-se notícias como esta abaixo que levam à reflexão sobre reeducação financeira pois alertam sobre a necessidade da busca por novas oportunidades de investimento.
...
Normalmente diversificamos nossa renda fixa também mas como gostaria de focar nas análises de renda variável utilizaremos o simulador disponível no site <https://verios.com.br/> neste [link](https://simulador-tesouro-direto.verios.com.br/) para selecionar apenas um título:
...
Note que a relação entre TUPY3 e ELET3 não é linear e além disso a correlação entre  é fraca, o que pode ser benéfico para o portfólio pois uma possível queda em uma não parece não ter tanto impacto na outra.
...
Portanto, ao valor de `r moeda_real(cot_inicio[3])` compramos `r qtd_inicio[3]` Bitcoin totalizando `r cot_inicio[3] * qtd_inicio[3]` completando nossa carteira.
...
* os valores de cotação e quantidade adquiridas de cada uma no momento da montagem da carteira;
...
<center>![](report_stocks.gif){width=70%}</center>
</br>
...
* Criar uma API para fornecer os resultados
